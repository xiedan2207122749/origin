...
public class SysImageUploadServiceImpl extends ServiceImpl<SysImageUploadDao, SysImageUploadEntity> implements SysImageUploadService {
    ...
    public void updateUsedByLink(String link) {
        this.update().set("use_flag", WhetherEnum.YES.getValue()).eq("pre_file_name", Long.parseLong((String) link.subSequence(link.lastIndexOf("/") + 1, link.lastIndexOf(".")))).last("limit 1").update();
    }
    ...
    public void removeByLink(String link) {
        this.remove(new QueryWrapper<SysImageUploadEntity>().eq("pre_file_name", Long.parseLong((String) link.subSequence(link.lastIndexOf("/") + 1, link.lastIndexOf(".")))).last("limit 1"));
    }
